...
void MMDModel::MaterialInit(std::string path)
{
	int offset = 0;
...
void MMDModel::SubProcessMorph(PmxMorph& pmxMorph, float weight)
{
...
void MMDModel::ProcessAnimation()
{
	for (Bone& bone : bones)
	{
		if (animation->boneMap.find(bone.name) != animation->boneMap.end())
		{
			...
			{
				if (i == 0)i++;
				MMDAnimation::BoneFrame& bf = bone_frames[i - 1];
				bone.trans = bf.position;
				bone.rot = bf.rotation;
			}
			else
			{
				MMDAnimation::BoneFrame& bf0 = bone_frames[i - 1];
				MMDAnimation::BoneFrame& bf1 = bone_frames[i];
...
void MMDModel::ProcessPhysics(float dt)
{
